...
import com.toornament.exception.Errors;
...
public class Webhooks extends Concept {
  ...
  public List<Webhook> getWebhooks(Map<String, String> range) {
    ...
    try {
      ...
      return mapper.readValue(
          responseBody, mapper.getTypeFactory().constructCollectionType(List.class, Webhook.class));
  ...
  public Webhook createWebhook(WebhookQuery query) throws IOException {
    HttpUrl.Builder url = new HttpUrl.Builder();
    url.scheme("https")
        .host("api.toornament.com")
        .addEncodedPathSegment("organizer")
    ...
    Request request = client.getAuthenticatedRequestBuilder().post(body).url(url.build()).build();
    ...
    try {
      ...
      logger.info(
          mapper.readValue(responsebody, mapper.getTypeFactory().constructType(Errors.class)).toString());
  ...
  public Webhook updateWebhook(String webhookID, WebhookQuery query) {
    ...
    url.scheme("https")
        ...
        .addEncodedPathSegment("webhooks");
    RequestBody body = RequestBody.create(MediaType.parse("application/json"), query.toString());
    Request request = client.getAuthenticatedRequestBuilder().patch(body).url(url.build()).build();
  ...
  public List<WebhookSubscription> getWebhookSubscriptions(
      String webhookID, Map<String, String> range) {
    HttpUrl.Builder url = new HttpUrl.Builder();
    url.scheme("https")
        .host("api.toornament.com")
        .addEncodedPathSegment("organizer")
  ...
  public WebhookSubscription createWebhookSubscription(
      String webhookID, WebhookSubscriptionQuery query) {
    HttpUrl.Builder url = new HttpUrl.Builder();
    url.scheme("https")
        .host("api.toornament.com")
        .addEncodedPathSegment("organizer")
    ...
    try {
      String responseBody = client.executeRequest(request).body().string();
      return mapper.readValue(
          responseBody, mapper.getTypeFactory().constructType(WebhookSubscription.class));
    } catch (IOException | NullPointerException e) {

      throw new ToornamentException(
          "Got IOException creating webhook subscription: " + e.getMessage());
    }
  }

  public WebhookSubscription updateWebhookSubscription(
      String webhookID, String webhookSubID, WebhookSubscriptionQuery query) {
    HttpUrl.Builder url = new HttpUrl.Builder();
    url.scheme("https")
        .host("api.toornament.com")
        .addEncodedPathSegment("organizer")
